...
## 🚫 กฎข้อที่ 20: ห้าม AI รัน Tests เองโดยเด็ดขาด

> **⛔ ABSOLUTE RULE — ห้ามละเมิดไม่ว่ากรณีใดก็ตาม**

**ผู้ใช้จะ Run Tests เอง — AI ห้ามรันเด็ดขาด ยกเว้นผู้ใช้สั่งโดยตรงเท่านั้น**

### ❌ คำสั่งที่ห้ามรันโดยไม่ได้รับคำสั่ง

- ❌ `npx playwright test` หรือ variant ใดก็ตาม
- ❌ `npm test`, `npm run test`, `npm run test:*`
- ❌ `.\scripts\test.ps1` หรือสคริปต์ test ใดๆ
- ❌ `npx playwright test --reporter=list` หรือ flag อื่นๆ
- ❌ รัน test แม้แต่บางส่วน (single file, single test)

### ✅ สิ่งที่ทำได้โดยไม่ต้องรอคำสั่ง

- ✅ แก้ไขโค้ด test (`.spec.js`)
- ✅ commit + push code
- ✅ อธิบายวิธีรัน test ให้ผู้ใช้ทราบ

### 🔴 เงื่อนไขเดียวที่รัน test ได้

> ผู้ใช้พิมพ์คำสั่งให้รัน test อย่างชัดเจน เช่น:
> "รัน test", "ลอง test", "test ให้หน่อย", "run playwright"

ถ้าไม่มีคำสั่งชัดเจน → **ห้ามรันเด็ดขาด**

### เหตุผล

- Tests ใช้เวลานาน (~2-4 นาที)
- ผู้ใช้ต้องการควบคุมเวลาที่จะรันเอง
- หลีกเลี่ยงการรัน tests ซ้ำโดยไม่จำเป็น
- Server (`http-server`) ต้องรันอยู่ก่อน — AI ไม่รู้สถานะ

---

---

## 🤔 กฎข้อที่ 23: การตัดสินใจเมื่อไม่แน่ใจ (Decision Under Uncertainty)

**หาก AGENT ไม่แน่ใจหรือไม่ชัวร์ ให้ทำมาเป็น choice เพื่อให้ user ตัดสินใจอีกที**

### ✅ รูปแบบการนำเสนอ Choice

```
เลือก choice ที่ต้องการ:

K - ทำตามที่ agent แนะนำ (บอก default ชัดเจน)
A - [ตัวเลือก A] - อธิบายสั้นๆ
B - [ตัวเลือก B] - อธิบายสั้นๆ
C - [ตัวเลือก C] - อธิบายสั้นๆ
```
...
| สถานการณ์ | Action |
| --- | --- |
| มีหลายวิธีแก้ปัญหา | เสนอ choice |
| ไม่แน่ใจว่า user ต้องการอะไร | ถาม choice |
| การเปลี่ยนแปลงกระทบหลายส่วน | เสนอ choice ก่อน |
| มี tradeoff ที่สำคัญ | แจ้ง tradeoff + เสนอ choice |

### ❌ สิ่งที่ห้ามทำ

- ❌ ห้ามเดาและทำไปเองเมื่อไม่แน่ใจ
- ❌ ห้ามถามมากกว่า 1 รอบโดยไม่จำเป็น (ถ้าถามแล้ว user ตอบ → ทำเลย)
- ❌ ห้ามเสนอ choice สำหรับงานที่ชัดเจนอยู่แล้ว

---

**อัปเดตล่าสุด:** 15-05-2026, 10:00 น.
**เวอร์ชัน:** V.2.1.0 (15-05-2026) - เพิ่มกฎ 23 ให้เสนอ choice เมื่อไม่แน่ใจ
